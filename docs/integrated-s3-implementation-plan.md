...
| multipart upload lifecycle | implemented | emulated | implemented | yes | initiate/upload-part/complete/abort plus bucket-level `GET ?uploads` listing/discovery are implemented on the current disk/Core/HTTP surface, and the native S3 provider now implements the same provider-facing lifecycle/listing natively through the AWS SDK; replicated write modes now stage multipart uploads on the primary backend and mirror the committed object on completion, with multipart discovery remaining primary-authoritative until completion |
...
- current multipart orchestration now supports both replicated write modes by keeping in-progress multipart state on the primary backend, then mirroring the committed object during `CompleteMultipartUpload`; bucket-level multipart discovery stays primary-authoritative until completion
...
- replica-write divergence is now surfaced through the repair backlog for both async replica dispatch and post-primary partial synchronous failures, including multipart completions that now mirror the finalized object through the same provider-agnostic repair pipeline
...
- Status update:
  ...
  - multipart uploads are now supported under both replicated write modes by staging initiate/upload-part/abort on the primary backend and mirroring the finalized object during completion through the existing write-through and async-repair paths; bucket-level multipart discovery is intentionally primary-authoritative while an upload is still in progress
- Current semantics and boundaries:
  ...
  - replicated multipart semantics intentionally treat in-progress uploads as primary-local state until completion; replicas do not expose staged uploads independently, and completed-object visibility on replicas follows the configured synchronous or async replication mode
